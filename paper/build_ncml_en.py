...
rich([("Abstract. ", {"bold": True}),
      ("Chat-based LLMs return free text; a new category, typed evaluation models (\"System "
       "One models\"), instead return schema-constrained typed objects. Jev (TypeSafe AI, "
       "launched September 2026) is the first commercial example. This paper tests, across six "
       "experiments on Jev, whether output collapsed to a single scalar can distinguish genuine "
       "conflict from genuine ignorance, as predicted by annotated decision theory. With Jev's "
       "simplest type (boolean/Noul), conflict and ignorance collapse to the same narrow band "
       "(0.46-0.57), consistent with that prediction. A Choice schema naming \"conflicting "
       "evidence\" and \"insufficient evidence\" as explicit options separates both cases "
       "perfectly (p=1.0); restricting Choice to two options without escape categories shows "
       "neither pattern, instead revealing an unanticipated directional bias tied to lexical "
       "cues. A Score-type rubric over \"how determinate is the evidence\" separates severe "
       "conflict from total absence (2.04 vs. 3.95 on a five-level scale) without naming the "
       "cause. Composing two independent boolean calls — for and against, without forcing them "
       "to sum to 1 — reconstructs the four canonical vertices of annotated paraconsistent "
       "logic's lattice, validated at n=20 per category across 20 heterogeneous domains. A "
       "sixth experiment tests whether a third neutrosophic indeterminacy coordinate adds "
       "information beyond that pair: it does, but with an effect an order of magnitude smaller "
       "than the conflict/non-conflict distinction, too small relative to single-case noise to "
       "be useful case by case. The central finding is that the same model preserves, destroys, "
       "or distorts the conflict/ignorance distinction depending on which question its schema "
       "forces, not on whether the output is scalar or categorical. Full code and data, "
       "reproducible with a reader's own AI Gateway key, are released publicly.", {})],
     "Abstract")
