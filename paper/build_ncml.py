...
rich([("Resumen. ", {"bold": True}),
      ("Los modelos de chat basados en LLM devuelven texto libre que debe interpretarse; una "
       ...
       "indistinguibles sin más contexto. Pero un segundo experimento, con el tipo Choice del "
       "mismo modelo y un esquema que nombra explícitamente \"evidencia en conflicto\" y "
       "\"evidencia insuficiente\" como opciones, separa ambos casos con probabilidad 1,0 en los "
       "cuatro casos completados. Un tercer experimento, con Choice restringido a las dos "
       "opciones originales (red/green, sin categorías de escape), no reproduce ni el colapso a "
       "~0,5 del primer experimento ni la separación perfecta del segundo: aparece un sesgo "
       "direccional hacia \"red\" (0,67–0,85 según el caso) que correlaciona con la presencia "
       "léxica de la palabra \"red\" en el estado, no de forma limpia con la categoría "
       "TORN/SILENT. El hallazgo central, por tanto, no es que Jev colapse per se, sino que el "
       "mismo modelo preserva, destruye, o distorsiona de forma no anticipada la distinción "
       "según el tipo de pregunta y el esquema que se le declare — un riesgo de diseño de "
       "interfaz más complejo de lo que un solo experimento de seguimiento podía anticipar. Un "
       "cuarto experimento exploratorio, con el tipo Score (un escalar ordinal sobre una "
       "rúbrica, distinto de boolean y de Choice), refina esto aún más: separa con claridad "
       "conflicto severo de ausencia total de evidencia (2,04 vs. 3,95 en una escala de cinco "
       "niveles), aunque no logra ordenar de forma confiable grados intermedios. Un quinto "
       "experimento muestra que un par independiente (μ, λ), como el de la lógica paraconsistente "
       "anotada (§4), puede reconstruirse sin que Jev lo calcule internamente: componiendo dos "
       "llamadas boolean separadas —una a favor, una en contra, sin forzar que sumen 1— se "
       "recuperan los cuatro vértices canónicos del retículo, y TORN se separa de SILENT con más "
       "nitidez que en el Experimento 1. Un sexto experimento prueba si una tercera coordenada de "
       "indeterminación (I), como en la lógica neutrosófica, aporta algo que (μ, λ) no capture; "
       "validado con un dataset de 20 dominios heterogéneos (n=20 por categoría), I distingue "
       "estadísticamente las tres categorías \"no determinadas\" entre sí (pruebas t de Welch, "
       "p<0,01 en las tres comparaciones), pero con magnitudes muy distintas: grande entre "
       "conflicto y las otras dos, y un orden de magnitud menor entre ausencia total y evidencia "
       "intrínsecamente vaga — una diferencia real a nivel de población, pero demasiado pequeña "
       "frente al ruido de un caso individual para ser operacionalmente útil caso por caso. Se "
       "discuten las implicaciones para quien construya sobre modelos de evaluación tipada, y se "
       "identifican preguntas abiertas concretas para trabajo futuro.", {})],
     "Abstract")
...
rich([("Abstract. ", {"bold": True}),
      ("Chat-based LLMs return free text that must be interpreted; a category of tools that "
       ...
       "band, indistinguishable without further context. But a second experiment, using the "
       "same model's Choice type with a schema that explicitly names \"conflicting evidence\" "
       "and \"insufficient evidence\" as options, separates both cases with probability 1.0 in "
       "all four completed cases. A third experiment, with Choice restricted to the two "
       "original options (red/green, no escape categories), reproduces neither the ~0.5 "
       "collapse of the first experiment nor the clean separation of the second: a directional "
       "bias toward \"red\" appears (0.67–0.85 depending on the case) that correlates with the "
       "lexical presence of the word \"red\" in the state, not cleanly with the TORN/SILENT "
       "category. The central finding, therefore, is not that Jev collapses per se, but that "
       "the same model preserves, destroys, or unpredictably distorts the distinction depending "
       "on the question type and the declared schema — an interface design risk more complex "
       "than a single follow-up experiment could anticipate. A fourth, exploratory experiment, "
       "using the Score type (an ordinal scalar over a rubric, distinct from boolean and "
       "Choice), refines this further: it clearly separates severe conflict from total absence "
       "of evidence (2.04 vs. 3.95 on a five-level scale), though it fails to reliably order "
       "intermediate degrees. A fifth experiment shows that an independent (μ, λ) pair, as in "
       "annotated paraconsistent logic (§4), can be reconstructed without Jev computing it "
       "internally: composing two separate boolean calls — one for evidence in favor, one "
       "against, without forcing them to sum to 1 — recovers the lattice's four canonical "
       "vertices, and separates TORN from SILENT more sharply than Experiment 1. A sixth "
       "experiment tests whether a third indeterminacy coordinate (I), as in neutrosophic "
       "logic, captures something (μ, λ) do not; validated on a dataset of 20 heterogeneous "
       "domains (n=20 per category), I statistically distinguishes all three \"undetermined\" "
       "categories from each other (Welch's t-tests, p<0.01 on all three comparisons), but with "
       "very different magnitudes: large between conflict and the other two, and an order of "
       "magnitude smaller between total absence and intrinsically vague evidence — a real "
       "population-level difference, but too small relative to single-case noise to be "
       "operationally useful case by case. Implications for anyone building on typed evaluation "
       "models are discussed, and concrete open questions for future work are identified.", {})],
     "Abstract")
